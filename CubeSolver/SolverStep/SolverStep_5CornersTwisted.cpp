#include "SolverStep_5CornersTwisted.h"
...
std::vector<PartialSolution> SolverStep_5CornersTwisted::Solve(const std::vector<CubeMove>& scramble, const Solution& solutionSoFar)
{
  Cube3x3x3 cube;
  cube += scramble;
  ...
  {
    throw std::domain_error(std::to_string(wrongCorners.size()) + " wrong corners when expecting 5 wrong corners");
  }
  ...
  for (size_t corner1Index = 0; corner1Index < 3; ++corner1Index)
  {
    const auto& corner1 = wrongCorners[corner1Index];
    for (size_t corner2Index = corner1Index + 1; corner2Index < 5; ++corner2Index)
    {
      for (const auto& corner2 : wrongCornerStickers[corner2Index])
      {
        for (size_t corner3Index = corner1Index + 1; corner3Index < 5; ++corner3Index)
        {
          if (corner2Index != corner3Index)
          {
            for (const auto& corner3 : wrongCornerStickers[corner3Index])
            {
              ...
              {
                partialSolutions.push_back(PartialSolution{ cycleSolution, CubeGroup::AB5C_3cycle });
              }
            }
          }
        }
