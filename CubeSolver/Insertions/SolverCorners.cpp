...
Solution SolveCorners(const Cube3x3x3& scrambledCube, const Solution& skeleton);
...
std::set<StickerPosition> FindWrongCorners(Cube3x3x3 scrambledCube, const Solution& skeleton)
{
...
Solution SolveCornersWith3Cycle(const Cube3x3x3& scrambledCube, const Solution& skeleton,
                                ...
                                const std::set<StickerPosition>& remainingCorners)
{
  ...
  for (const auto& orientedCycleCornerC : remainingCorners)
  {
    ...
    for (const auto& cycleCornerC : cycleCornerCCandidates)
    {
      ...
      auto solution = SolveCorners(scrambledCube, cycleSolution);
...
Solution SolveCornersWithPairOf3Cycles(const Cube3x3x3& scrambledCube, const Solution& skeleton,
                                       StickerPosition cycleCornerA, StickerPosition cycleCornerB,
                                       const std::set<StickerPosition>& remainingCorners)
{
  Cube3x3x3 cube = scrambledCube;
  cube += skeleton.GetMoves();
  ...
  for (const auto& orientedCycleCornerC : remainingCorners)
  {
    ...
    for (const auto& cycleCornerC : cycleCornerCCandidates)
    {
      ...
      if (newWrongCorners.size() == 3 && !AreCornersCorrectlyPermuted(newCube, newWrongCorners))
      {
        ...
        auto cycleSolution = Corner3Cycle::InsertCorner3CycleInSkeleton(skeleton, cycle);
        auto solution = Corner3Cycle::SolveCorner3Cycle(scrambledCube, cycleSolution);
        if (!haveSolution || solution.Length() < bestSolution.Length())
        {
          haveSolution = true;
          bestSolution = solution;
...
Solution SolveL4C(Cube3x3x3 scrambledCube, const Solution& skeleton)
{
...
// Returns the new solution.
Solution SolveL2C(Cube3x3x3 scrambledCube, const Solution& skeleton)
{
  auto wrongCorners = FindWrongCorners(scrambledCube, skeleton);
  ...
  for (const auto& corner1 : wrongCorners)
  {
    ...
    if (!CornerStructure::AreStickersOnSameCubie(corner1, corner2))
    {
      ...
      else
      {
        ...
        auto cycleSolution = Corner3Cycle::InsertCorner3CycleInSkeleton(skeleton, cycle);
        solution = SolveCorners(scrambledCube, cycleSolution);
      }
      if (!haveSolution || solution.Length() < bestSolution.Length())
      {
        haveSolution = true;
