...
static const StickerPosition cornerCubie{ Face::Right, 8 };

// If the FR edge is solved, then F face moves would also be possible, but this would greatly
...
SolverEdgesFrontSlot::SolverEdgesFrontSlot(std::uint32_t maxMoves, const std::string& cacheFileName)
: solver(
    maxMoves,
    // Get key
    [](const Cube3x3x3& cube)
    {
      return SolverEdgesFrontSlot::GetKeyValue(cube);
    },
    possibleMoves,
    cacheFileName)
{}

std::vector<CubeMove> SolverEdgesFrontSlot::Solve(const Cube3x3x3& cube) const
{
  return solver.Solve(cube);
}

std::vector<CubeMove> SolverEdgesFrontSlot::Solve(const std::vector<CubeMove>& scramble) const
{
  Cube3x3x3 cube;
  cube += scramble;
  return Solve(cube);
...
std::uint32_t SolverEdgesFrontSlot::GetKeyValue(const Cube3x3x3& cube)
{
  // 7 edges, each:
