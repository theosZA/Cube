...
// A solver that finishes all remaining edges where the first 2 layers are done except for the front-right slot and
// edges are already oriented.
class SolverEdgesFrontSlot
{
public:
  ...
  SolverEdgesFrontSlot(std::uint32_t maxMoves, const std::string& cacheFileName = "");

  // Returns the move sequence that will solve the remaining edges where F2L is done except FDR and FR.
  ...
  // Returns the move sequence that will solve the remaining edges where F2L is done except FDR and FR from the given scramble.
  // If this can't be achieved within the number of moves specified in the constructor then an exception is thrown.
