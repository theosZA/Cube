...
// A solver that finishes the first 2 layers except for the back-right slot from a 2x2x3 block where edges are oriented.
class SolverF2LMinusBackSlot
{
public:
  ...
  SolverF2LMinusBackSlot(std::uint32_t maxMoves, const std::string& cacheFileName = "");

  // Returns the move sequence that will take a 2x2x3 block (with EO) to F2L solved except BDR and BR.
  ...
  // Returns the move sequence that will take a 2x2x3 block (with EO) to F2L solved except BDR and BR from the given scramble.
  // If this can't be achieved within the number of moves specified in the constructor then an exception is thrown.
  std::vector<CubeMove> Solve(const std::vector<CubeMove>& scramble) const;

private:
  // Returns a key value that distinguishes based on the position of cubies in the FRD 2x2x1 block.
